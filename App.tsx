...
import { getFeaturedContent, getHeroContent, getNewReleases, getCollections } from './services/movieService';
import { Movie, Collection } from './types';
import { AnimatePresence, motion } from 'framer-motion';

const App: React.FC = () => {
  // ─── Data State ────────────────────────────────────────────────────────────
  const [movies, setMovies] = useState<Movie[]>([]);           // Library grid
  const [heroMovies, setHeroMovies] = useState<Movie[]>([]);   // Hero slider
  const [newReleases, setNewReleases] = useState<Movie[]>([]); // New Releases tab
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        // Load hero + library in parallel first (fast initial paint)
        const [fetchedHero, fetchedMovies] = await Promise.all([
          getHeroContent(),
          getFeaturedContent(),
        ]);
        setHeroMovies(fetchedHero);
        setMovies(fetchedMovies);
        setFilteredMovies(fetchedMovies);
        setCollections(getCollections());

        // Load new releases in the background — doesn't block render
        const fetchedNew = await getNewReleases();
        setNewReleases(fetchedNew);
      } catch (e) {
  ...
  useEffect(() => {
    const filterContent = () => {
      // "New Releases" category has its own dedicated dataset
      if (activeCategory === 'New Releases') {
        let base = [...newReleases];
      ...
      // 'Recommended' shows all with no extra filter
  ...
  }, [activeTab, activeCategory, movies, newReleases]);
    ...
    <div className="min-h-screen bg-[#09090b] text-white font-sans selection:bg-yellow-500 selection:text-black">
      ...
      <main className="pl-0 md:pl-20">
        {activeTab !== 'COLLECTIONS' ? (
          <>
            {/* ── Hero Slider (2024 blockbusters) ── */}
            <div className="fixed top-0 left-0 right-0 z-0">
              <Hero movies={heroMovies.length > 0 ? heroMovies : movies.slice(0, 6)} onPlay={setCurrentPlayerMovie} />
            </div>
